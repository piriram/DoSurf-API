...
def run_collection():
                ...
                if has_kma and has_marine:
                    ...
                    if picked:
                        if forecast_count >= EXPECTED_FORECAST_HOURS * 0.9:
                            ...
                            partial_updates += 1
                            print("   ⚠️ 부분 데이터 저장 완료 (다음 실행 때 보완)")
                    else:
                        partial_updates += 1
                        print("   ⚠️ 부분 저장 (Open-Meteo 위주)")
                elif has_kma:
                    partial_updates += 1
                    print("   ⚠️ 부분 저장 (KMA만, Open-Meteo 실패)")
                else:
                    partial_updates += 1
                    print("   ⚠️ 부분 저장 (Open-Meteo만, KMA 서버 오류)")
            else:
    ...
    if partial_updates > 0:
        ...
        print("      💡 부분 데이터는 다음 실행 때 자동 보완됩니다")
    ...
    if partial_updates > 0:
        ...
        print("   - merge=True 옵션으로 기존 데이터와 자동 병합됩니다")
    ...
    alert_messages = []
    if failed_updates > 0:
        alert_messages.append(f"수집 실패 해변 {failed_updates}개 발생 (전체 {len(locations)}개 중)")
    if cleanup_error:
