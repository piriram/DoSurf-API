...
### 확인 결과 (2026-08-29, `KMA_API_KEY` 있는 로컬 환경에서 실측)

**`WAV`는 거의 항상 있다. → P0 확정.**

전체 해변(32곳)의 실시간 발표를 조회하면 모든 지역·모든 카테고리(TMP/WSD/SKY/…)가
동일하게 66/66 채워진다 — `WAV`만 빠지는 경우는 없다. 즉 iOS는 사실상 항상
기상청 `wave_height`를 쓰고, Phase 1(Open-Meteo)의 개선은 화면에 거의 도달하지 않는다.

더 나아가 **기상청 `WAV` 원본값 자체를 찍어보니 사실상 상수에 가깝다.**

```
속초/제주: 발표 시각 내 6개 fcstTime 전부 0.5로 고정
강릉/부산: 발표 시각 내 6개 fcstTime 전부 0으로 고정
```

같은 시각 Firestore에 저장된 두 값을 대조한 결과 (4개 지역, 최근 2개 타임스탬프):

| 지역 | 기상청 `wave_height` | Open-Meteo `om_wave_height` |
|---|---|---|
| 속초(6001) | 0.5 (고정) | 0.96 ~ 1.08 |
| 강릉(5001) | 0.5 (고정) | 0.92 ~ 0.96 |
| 제주(3001) | 0.5 (고정) | 1.40 (고정) |
| 부산(4001) | 0.5 (고정) | 0.88 ~ 0.90 |
| 포항(2001) | 0.5 (고정) | 0.90 ~ 0.94 |

기상청 단기예보 `WAV`는 5km 육상 격자 기반이라 실제 해양 파랑모델이 아니고,
관측된 값도 `0` 또는 `0.5` 두 가지뿐이라 **파싱 버그가 아니라 소스 자체의 한계**로 보인다
(원본 API 응답의 `fcstValue`를 직접 찍어서 확인함 — 가공 단계 문제 아님).

**결론: 기상청 파고는 사실상 정보량이 거의 없는 상수에 가깝다.** Open-Meteo가 유일하게
쓸만한 파고 소스이므로 뒤집기는 "혹시 몰라 확인" 수준이 아니라 **필요한 수정.**
...
값 차이가 실측으로 확인됐으므로(위 표) 뒤집으면 화면 파고 값이 지역에 따라
0.5 → 0.9~1.4로 눈에 띄게 올라간다. 사용자 공지나 온보딩 문구가 필요할 수 있다.
...
> **[2026-08-30 보강]** 이 차이의 상당 부분이 **모델 오차가 아니라 파주기 정의 차이**로 보인다.
>
> 파주기에는 평균주기와 첨두주기가 따로 있다. Open-Meteo `wave_period` 는 평균주기
> 계열이고, Windfinder·서핑 앱이 화면에 쓰는 건 첨두주기다. 첨두 쪽이 구조적으로 크다.
>
> Copernicus Marine이 둘을 따로 주길래 같은 지점·같은 날로 재봤다
> (제주 2026-08-30, Windfinder 8.0초 고정):
>
> | | 범위 | Windfinder 대비 |
> |---|---|---|
...
>
> 원시 MAE로 첨두 0.81초 · 평균 2.82초 — **3.5배 차이**다. 위 본문의 "5.4~7.3초"가
> 정확히 평균주기 대역이라는 점도 맞아떨어진다.
>
> **표본이 1일이라 확정은 아니다.** 그날 Windfinder 제주가 8.0초 상수여서 상관계수를
> 낼 수 없었고, 편향제거 순위도 상수 기준에서는 의미가 약하다. 다만 "모델을 바꿔야
> 한다"는 결론으로 가기 전에 **어느 정의의 파주기를 쓸 것인지부터 정해야 한다**는 건
> 분명해졌다. 재현: `.venv/bin/python3 -m scripts.model_compare --spot jeju
> --from-windfinder --models cmems,cmems_peak`
