...
- **데이터셋 ID**: `cmems_mod_glo_wav_anfc_0.083deg_PT3H-i` (2026-08-31 카탈로그 확인. `describe` 는 인증 없이 조회된다)
- **파주기가 둘이다**: `VTM10`(평균주기) 과 `VTPK`(첨두주기)를 따로 준다. Open-Meteo `wave_period` 는 평균주기 계열이고, Windfinder·서핑 앱이 화면에 쓰는 건 첨두주기다 — **지금까지의 파주기 불일치가 모델 차이가 아니라 정의 차이일 수 있다**
...
- **접근 방식**: REST 단건 조회 불가. `copernicusmarine` 파이썬 툴박스로 접근한다

> **[2026-08-31 정정]** "NetCDF-4 파일을 내려받아 직접 파싱해야 함"은 툴박스 v1 기준이었다.
> v2.4.1 에서 `open_dataset()` 이 ARCO(zarr) 저장소를 **lazy 하게** 열어주므로,
> 필요한 격자 상자와 시간 범위만 잘라 받으면 된다. 파일 다운로드·파싱 파이프라인이
> 필요 없다. 실제로 붙여봤고 코드는 `scripts/copernicus.py` 다.
> 아래 "정리"의 통합 작업량 판단도 그만큼 낮춰 읽어야 한다.
>
> 대신 새로 드러난 제약이 있다:
> - **수온·조석이 이 제품에 없다.** 파랑 전용이라 수온은 Open-Meteo에 계속 의존해야 한다
> - **`cell_selection=sea` 에 해당하는 옵션이 없다.** 가장 가까운 격자가 육지면 NaN이
>   오므로 바다 칸을 직접 골라야 한다
> - 무료 계정 가입과 `copernicusmarine login` 이 필요하다 (Open-Meteo는 키가 없다)
> - 의존성이 무겁다 — xarray·zarr·dask·numpy·pandas 로 `.venv` 가 540MB 가 된다.
>   그래서 `requirements-dev.txt` 로 분리했고 Cloud Run 이미지에는 넣지 않는다
