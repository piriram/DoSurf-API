...
> 1·2번은 로컬 자격증명으로 2026-08-29에 검증 완료 (결과는 각 항목 하단 참고).
> **남은 건 3번(수집 1회 실행)뿐** — 급하지 않음, 어차피 스케줄러가 3시간마다 자동으로 함.
...
**✅ 결과 (2026-08-29 18:02 KST)**: `고른 발표` = `17:00` (기대대로 직전 발표를 정확히 고름).
17:00 발표가 아직 안 올라와 `실제 사용`은 `14:00`로 폴백 — 정상 동작 확인. 9시간 밀림 버그 없음.
...
**✅ 결과 (2026-08-29)**: **WAV가 32개 해변 전부 100% 채워짐** — 다른 카테고리(TMP/WSD 등)와 동일한
채움률. **P0 확정.** 게다가 기상청 `WAV` 원본값 자체가 지역별로 `0` 또는 `0.5`로 6시간 내내 고정인
것도 원본 API 응답에서 직접 확인함(파싱 문제 아님, 소스 데이터의 한계). 같은 시각 Firestore 대조
결과와 상세 근거는 [`ios-migration.md` P0](./ios-migration.md#p0--파고-우선순위를-뒤집어야-한다) 참고.
